...
@RestController // tells Spring Boot that this class is a REST controller, meaning it will handle HTTP requests (GET, POST, etc.) and return data in JSON format
...
public class UserController { // this is the class responsible for managing users (User) in the application
    ...
    @PostMapping("/user") // responds to POST requests sent to http://localhost:8080/user
